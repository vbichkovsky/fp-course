...
jsonString = between (is '"') (is '"') (list $ (noneof "\"\\") |||
                                        (is '\\' >>> (parseSpec ||| hexu)))

parseSpec :: Parser Char
parseSpec = flbindParser character (\c -> case toSpecialCharacter c of
                                       Full s -> valueParser $ fromSpecialCharacter s
...
jsonObject = betweenSepbyComma '{' '}' parseAssoc where
  parseAssoc = jsonString >>= (\k -> spaces >>> (charTok ':') >>>
                                     jsonValue >>= (\v -> pure (k,v)))
...
jsonValue = (jsonNull >>= (const $ pure JsonNull)) |||
            (jsonTrue >>= (const $ pure JsonTrue)) |||
            (jsonFalse >>= (const $ pure JsonFalse)) |||
            (jsonString >>= (pure . JsonString)) |||
            (jsonNumber >>= (\n -> pure $ JsonRational False n)) |||
            (jsonArray >>= (\lst -> pure $ JsonArray lst)) |||
            (jsonObject >>= (\a -> pure $ JsonObject a))

-- | Read a file into a JSON value.
--
-- /Tip:/ Use @System.IO#readFile@ and `jsonValue`.
readJsonValue :: Filename -> IO (ParseResult JsonValue)
readJsonValue filename = readFile filename >>= pure . parse jsonValue
