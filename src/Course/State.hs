...
instance Functor (State s) where
  ...
  (<$>) f sa = State $ (\(a, s) -> (f a, s)) . runState sa
...
instance Applicative (State s) where
  ...
  (<*>) sf sa = State $ (\(f,s') -> runState (f <$> sa) s') . runState sf
...
instance Monad (State s) where
  ...
  (=<<) f sa = State $ (\(a, s) -> runState (f a) s) . runState sa
...
findM p = foldRight folder (pure Empty) where
  folder x facc = p x >>= (\b -> if b then pure (Full x) else facc)
...
firstRepeat l = eval (findM fun l) S.empty where
  fun a = State $ \s -> (S.member a s, S.insert a s)
...
distinct l = eval (filtering p l) S.empty where
  p a = State $ \s -> (not $ S.member a s, S.insert a s)
...
isHappy = contains 1 . firstRepeat . produce sumOfSquares . fromInteger where
  sumOfSquares x = sum $ map ((\d -> d*d) . digitToInt) (show' x)
