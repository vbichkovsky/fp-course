...
asMaybeZipper :: (ListZipper a -> MaybeListZipper a) -> MaybeListZipper a
  -> MaybeListZipper a
...
findLeft p (ListZipper l m r) = case break p l of
  (_, Nil) -> IsNotZ
  (x, h :. t) -> IsZ $ ListZipper t h (reverse x ++ m :. r)
...
findRight p (ListZipper l m r) = case break p r of
  (_, Nil) -> IsNotZ
  (x, h :. t) -> IsZ $ ListZipper (reverse x ++ m :. l) h t
...
moveLeftLoop (ListZipper (h :. t) m r) = ListZipper t h (m :. r)
moveLeftLoop (ListZipper Nil m r) = let (h :. t) = reverse (m :. r)
...
moveRightLoop (ListZipper l m (h :. t)) = ListZipper (m :. l) h t
moveRightLoop (ListZipper l m Nil) = let (h :. t) = reverse (m :. l)
...
moveLeftN' num lz = fun (num, 0, IsZ lz) where
  fun (_, a, IsNotZ) = Left (a - 1)
  fun (n, a, IsZ l)
    |n > 0 = fun (n - 1, a + 1, moveLeft l)
    |n < 0 = fun (n + 1, a + 1, moveRight l)
    |otherwise = Right l
...
nth n lz
  | n < 0 = IsNotZ
  |otherwise = let l = toList lz
                   left = reverse $ take n l
               in case drop n l of
                    (h:.t) -> IsZ $ ListZipper left h t
                    _ -> IsNotZ
...
end (ListZipper l m (h:.t)) = end (ListZipper (m:.l) h t)
...
start (ListZipper (h:.t) m r) = start (ListZipper t h (m:.r))
...
-- /Tip:/ Use `zipWith`
  (ListZipper fl f fr) <*> (ListZipper l m r) =
    ListZipper (zipWith ($) fl l) (f m) (zipWith ($) fr r)
...
instance Extend ListZipper where
  f <<= lz = let fun (IsZ l) = Full (f l, l)
                 fun IsNotZ = Empty
                 left = unfoldr (fun . moveLeft) lz
                 right = unfoldr (fun . moveRight) lz
             in ListZipper left (f lz) right
...
instance Traversable ListZipper where
  traverse f (ListZipper l m r) = let fl = traverse f l
                                      fr = traverse f r
                                  in lift3 ListZipper fl (f m) fr
