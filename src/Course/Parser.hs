...
valueParser val = P (\input -> Result input val)
...
mapParser f p = P (\input -> case parse p input of
...
list1 p = flbindParser p (\h -> flbindParser (list p) (\t -> valueParser (h:.t)))
...
satisfy cond = bindParser fun character where
  fun c = if cond c then valueParser c else unexpectedCharParser c
...
-- True
natural :: Parser Int
natural = bindParser fun (list1 digit) where
  fun chars = case read chars of
    Full num -> valueParser num
    Empty -> failed
...
sequenceParser = foldRight prepend (valueParser Nil) where
  prepend :: Parser a -> Parser (List a) -> Parser (List a)
  prepend p ps = flbindParser p (\h -> flbindParser ps (\t -> valueParser (h:.t)))
...
firstNameParser = flbindParser upper (\h -> flbindParser (list lower)
                                       (\t -> valueParser (h:.t)))
...
surnameParser = flbindParser upper (\h -> flbindParser (thisMany 5 lower)
                                     (\m -> flbindParser (list lower)
                                       (\t -> valueParser (h:.m ++ t) )))
...
phoneParser = flbindParser digit (\h -> flbindParser phoneBodyParser
                                   (\t -> flbindParser (is '#')
                                     (\_ -> valueParser (h:.t))))
...
personParser = flbindParser ageParser
  (\yrs -> wsAnd firstNameParser
    (\fname -> wsAnd surnameParser
      (\sname -> wsAnd smokerParser
        (\smkr -> wsAnd phoneParser
          (\phn -> valueParser (Person yrs fname sname smkr phn)))))) where
  wsAnd p f = spaces1 >>> bindParser f p
