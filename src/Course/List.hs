...
filter p = foldRight fun Nil where
...
seqOptional = foldRight fun (Full Nil) where
  fun :: Optional a -> Optional (List a) -> Optional (List a)
  fun Empty _ = Empty
  fun _ Empty = Empty
  fun (Full x) (Full xs) = Full (x :. xs)
...
notReverse = reverse -- because impossible otherwise
